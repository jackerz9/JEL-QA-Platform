...
const ROLE_LABELS = {
  ...
  viewer: { label: 'Viewer', color: 'bg-slate-600/30 text-slate-400 border-slate-600/30' },
};
    ...
    <div>
      ...
      <div className="flex gap-4 mb-4">
        <div className="flex items-center gap-2 text-xs text-slate-400">
          <Shield size={12} className="text-red-400" /> Admin: acceso total
        </div>
        <div className="flex items-center gap-2 text-xs text-slate-400">
          <UsersIcon size={12} className="text-blue-400" /> Supervisor: evaluar, reportes, contactos
        </div>
        <div className="flex items-center gap-2 text-xs text-slate-400">
          <Eye size={12} className="text-slate-400" /> Viewer: solo lectura
        </div>
      </div>
      ...
      {showAdd && (
        <div className="card mb-4">
          <div className="grid grid-cols-6 gap-3">
            ...
            <div>
              <label className="text-xs text-slate-400 block mb-1">{editing ? 'Nueva contraseña' : 'Contraseña'}</label>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {users.map(u => {
              ...
              return (
                <tr key={u._id} className={`border-b border-slate-800 ${!u.active ? 'opacity-40' : ''}`}>
                  ...
                  <td className="py-2.5">
                    <span className={`badge ${u.active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-600/30 text-slate-500'}`}>
                      {u.active ? 'Activo' : 'Inactivo'}
                    </span>
                  </td>
                  <td className="py-2.5 text-xs text-slate-500">{u.lastLogin ? new Date(u.lastLogin).toLocaleString('es-CL') : 'Nunca'}</td>
                  <td className="py-2.5 flex gap-2">
                    <button onClick={() => handleEdit(u)} className="p-1.5 rounded hover:bg-slate-700 text-slate-400 hover:text-gray-200"><Pencil size={14} /></button>
                    {u._id !== currentUser?.id && (
                      <button onClick={() => handleDelete(u._id)} className="p-1.5 rounded hover:bg-slate-700 text-slate-400 hover:text-red-400"><Trash2 size={14} /></button>
                    )}
                  </td>
                </tr>
              );
